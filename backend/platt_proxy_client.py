...
class Client(object):
    ...
    async def watch_download_request_queue(self):
        ...
        cl.debug("Requested file data and hash")
    ...
    async def _download_and_return_file(self, requested_file):
        ...
        while True:
            try:
                # open a connection
                cl.info("Attempting to open a connection for requesting files")
                ...
                counter += 1
                if counter == 3:
                    cl.warning("Too many timeouts")
                    break

                cl.warning("Can't establish a connection to request files, "
                             "waiting a bit")
                await asyncio.sleep(3.5)
